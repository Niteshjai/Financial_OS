...
/**
 * Hash an email address with SHA-256 + salt for lookup.
 * The raw email address is NEVER stored.
 */
export function hashEmail(email: string): string {
  const salt = getAadhaarSalt();
  const normalized = email.trim().toLowerCase();
  return crypto.createHash('sha256').update(`${salt}:email:${normalized}`).digest('hex');
}
