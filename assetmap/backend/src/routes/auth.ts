...
const authRoutes: FastifyPluginAsync = async (fastify, opts) => {
  ...
  const emailOtpStore = new Map<string, { email: string; otp: string; expiresAt: number; used: boolean }>();

  // Registration store — holds pending registrations after OTP verification
  const registrationStore = new Map<string, { phone: string; countryCode: string; createdAt: number; expiresAt: number; kycData?: any; email?: string }>();
  ...
  fastify.post('/email/initiate', {
    ...
    try {
      const { registrationToken, email } = request.body as any;
      ...
      const otp = String(Math.floor(100000 + Math.random() * 900000));
      emailOtpStore.set(registrationToken, {
        email,
        otp,
  ...
  fastify.post('/email/verify', {
    ...
    try {
      const { registrationToken, otp } = request.body as any;
      ...
      const txn = emailOtpStore.get(registrationToken);
      if (!txn) {
        return reply.status(400).send(errorResponse(ERROR_CODES.OTP_EXPIRED, 'OTP has expired. Please request a new one.'));
      }
      if (txn.used) {
      ...
      if (Date.now() > txn.expiresAt) {
        emailOtpStore.delete(registrationToken);
