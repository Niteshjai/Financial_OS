...
export default function SMSSetup() {
  ...
  async function handleSendOTP() {
    ...
    setLoading(true);
    try {
      await api.post('/2fa/sms/begin-setup');
      setStep('verify');
  ...
  async function handleVerify() {
    ...
    setVerifying(true);
    try {
      await api.post('/2fa/sms/confirm-setup', { code });
    ...
    <div className="min-h-screen text-zinc-900 font-sans pb-20 bg-zinc-50/50">
      ...
      <main className="max-w-[576px] mx-auto px-6 mt-12">
        <div className="mb-10 text-center">
          <h1 className="text-3xl font-display font-light tracking-tight text-zinc-900">Set Up SMS 2FA</h1>
          <p className="text-zinc-600 mt-2">Receive login codes directly to your registered mobile number.</p>
        </div>
        ...
        {step === 'intro' && (
          <div className="bg-white rounded-[24px] shadow-sm border border-zinc-200/50 p-8 flex flex-col items-center">
            ...
            <p className="text-sm text-zinc-500 mb-8 text-center max-w-[384px]">
              We will send a 6-digit code to your registered mobile number to verify that you can receive SMS messages.
            </p>
