...
export async function initiateEmail(registrationToken: string, email: string) {
  const res = await api.post<ApiResponse<{ message: string; expiresInSeconds: number }>>(
    '/auth/email/initiate',
...
export async function verifyEmail(registrationToken: string, otp: string) {
